    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="p-3 rounded-xl bg-slate-800/60 border border-slate-700/60 text-xs">
          <div className="text-slate-400">Current Gross Baseline:</div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1">Allowances ($)</label>
            <input
              type="number"
              min="0"
              step="50"
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1">Deductions ($)</label>
            <input
              type="number"
              min="0"
              step="50"
        ...
        <div className="p-3 rounded-xl bg-indigo-950/40 border border-indigo-500/30 text-xs flex justify-between items-center">
          <div>
            <span className="text-indigo-300 block font-semibold">New Gross: ${grossSalary.toLocaleString()}</span>
            <span className="text-slate-400">Total Deductions: -${deductions.toLocaleString()}</span>
          </div>
          <div className="text-right">
            <span className="text-[10px] text-slate-400 block uppercase font-bold">New Net Take-Home</span>
            <span className="text-base font-extrabold text-emerald-400">${netSalary.toLocaleString()}</span>
          </div>
        </div>

        <div>
          <label className="block text-xs font-semibold text-slate-300 mb-1">Reason / Revision Notes</label>
          <input
            type="text"
